...
Stakeholder.getStakeholderbyName = (name, result) => {
    Connection.query("SELECT NAME, CONTACT, STREET, MAILING, PHONE, CONTACTED, ATTEMPTS, CONSULTATION, FOLLOWUP FROM wascana WHERE NAME = ?", name, (err, res) => {
        if (err) {
            console.log('error');
            result(null, err);
        } else {
...
Stakeholder.updateStakeholder = (stakeholderData, result) => {
    Connection.query("UPDATE wascana set NAME = ?, CONTACT = ?, STREET = ?, MAILING = ?, PHONE = ?,  CONTACTED = ?, ATTEMPTS = ?, CONSULTATION = ?, FOLLOWUP = ? WHERE name = ?", 
